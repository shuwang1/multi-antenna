...
R\right)\right)&=&\left[1-\cos^{(2m-2)}\left(\psi\right)\left(\psi\right)\right]S_{2m}\left(R\right)
\end{array}.
\end{equation}
...
\end{figure}
\noindent With matching the sum of the sphere-cap areas with the
...
\end{array}
\end{equation}
\noindent where $\bh_{n}(l)=\left[h_{n,1}(l)\ h_{n,2}(l)\ \ldots\
h_{n,M}(l)\right]^{\rm T}$ for $1\leq l \leq L_{c}$ is the channel
impulse response vector from the $m$th transmit antenna to the $N$
receive antennas and $L_{c}$ is the maximum channel order for all
$N$ subchannels. $s_m(t)$ denotes the transmitted symbol from the
$m$th antenna. Due to the commutativity of convolution, the
...
designs: (i) {\em orthogonal design} by {time-multiplexed pilot
pattern}, and (ii) {\em non-orthogonal design} by {superimposed
pilot pattern}.
...
s_{d}}^{\ast}}\right|^2\right\}$. The MSE of
...
that the channel is constant during estimation, it is independent
