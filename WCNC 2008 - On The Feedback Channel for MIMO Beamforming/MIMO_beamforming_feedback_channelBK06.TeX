...
reverselink channel affect the reliability of feedback channel and
therefore system throughput with finite-rate feedback.

The analysis of MIMO feedback channel is an example of joint
source-channel coding problem. In this paper, the distortion and
reliability of MIMO feedback channel are discussed with
considering forwardlink design and reverselink channel condition.
The forwardlink for channel estimation is modelled as a Gaussian
channel with pilot and data signals transmitted. On MIMO
forwardlink design, the Cramer-Rao lower bound (CRLB) of channel
estimation is analyzed and presented, respectively, for
time-multiplexed pilots (TMP) or SIP. The MIMO feedback channel in
reverselink is modelled as a binary erasure channel with additive
Gaussian noise. The binary erasure channel is a popular and
simplified model for fading channel, especially for reverselink
feedback channels. With this noisy Gaussian binary erasure
feedback channel model, it shows that the actual transmit rate of
feedback channel is lower bounded by the channel quantization and
upper bounded by the transmit reliability.  Therefore some
tradeoff for feedback channel design are revealed.
...
\begin{figure}
\center{
\includegraphics[width=3.0in, angle=0]{Noisy_Gaussian_Feedback_Channel2.eps}
...
$\left(R,\ D_{\bh}\right)$ with the squared error distortion
\begin{equation}
\begin{array}{rcl}
D\left(R\right)&=&\sum\limits_{m=1}^{M}\left(w_{i,m}-v_{i,m}\right)^2
\end{array}\label{squared_error_distortion}
\end{equation}
\noindent is of the most interests.




is defined with the squared error rate distortion of channel
quantization
\begin{equation}
\begin{array}{l}
D_{\phi}\left(R\right)=\sum\limits_{m=1}^{M}\left(w_{i,m}-v_{i,m}\right)^2=2\left(1-\alpha^{2}\right)
\end{array},\label{squared_error_distortion}
\end{equation}
\noindent where $\alpha=\bw_{i}^{\rm H}\bv_{i}$. And the
achievable rate reliability pair $\left(R,\ \epsilon\right)$ is
defined with the error exponent $\epsilon$ denoting how fast the
reverselink bit-error rate (BER) $P_{e}$ decays for the transmit
rate $R$ and which can be expressed by
...
how the imperfect channel estimation, quantization and channel
condition affect the rate distortion and reliability of MIMO
feedback channel.

\section{MIMO Channel Quantization Distortion}
In general, the lower bound to the mean-squared errors (MSE) of
...
If the MSE of forwardlink channel estimation is
$\sigma_{\mbox{\tiny FL}}^{2}$, the minimum rate at distortion
$D_{\bh}$ is given by
\begin{equation}\hspace{-0.10in}
\begin{array}{rcl}
R(D_{\bh})&=&\sum\limits_{i=1}^{N}\min\limits_{E\{d(\bh_{i},\
\hat{\bh}_{i})\leq D_{\bh}\}} I\left(\bh_{i};\
\hat{\bh}_{i}\right)\\
&\geq&\frac{N}{2}\log_{2}\left(\frac{\sigma_{\hat{h}}^{2}}{D_{\bh}}\right)\\
&=&\frac{N}{2}\log_{2}\left(\frac{\sigma_{\mbox{\tiny
FL}}^{2}+\sigma_{h}^{2}}{D_{\bh}}\right)\ .
\end{array}
\end{equation}
\begin{figure}
\center{
...
For a $M$-dimensional vector channel response $\bh=\left[h_{1}\
h_{2}\ \ldots\ h_{M}\right]^{\rm T}$, the relationship between
Cartesian coordinate system and spherical coordinate system is
defined by
\begin{equation}%\hspace{-0.20in}
\begin{array}{rcl}
h_{m}&=&r\Phi_{m}
\end{array}\label{coordinate}
\end{equation}
\noindent with
\begin{equation}\hspace{-0.20in}
\begin{array}{rcl}
\Phi_{m}&=&\begin{cases} \cos\phi_{m-1}\prod\limits_{i=1}^{m-2}\sin(\phi_{1})& m < M\\
\sin\phi_{M-1}\prod\limits_{i=1}^{M-2}\sin(\phi_{i})& m= M
\end{cases}
\end{array}.\label{phi_coordinate}
\end{equation}
\noindent The Jacobian transformation of (\ref{coordinate}) is
\begin{equation}%\hspace{-0.20in}
\begin{array}{rcl}
\frac{\partial }{\partial \left(r,\
\phi_{i}\right)}h_{m}&=&\left[\Phi_{m}\ \
r\frac{\partial}{\partial\phi_{i}}\Phi_{m}\right]
\end{array}.\label{Jacobian}
\end{equation}
\noindent The total squared error rate distortion $D_{\bh}$ can be
decomposed into
\begin{equation}%\hspace{-0.20in}
\begin{array}{rcl}
D_{\bh}&\leq&D_{r}+r^{2}D_{\phi}
\end{array}
\end{equation}
\noindent with $D_{r}$ denoting the rate distortion of channel
amplitude quantization $r=\left\|\bh\right\|$ and $D_{\phi}$
denoting the phase distortion of $\sqrt{1-\alpha^2}$.



\begin{lemma}(Cramer-Rao Lower Bound) For the time-multiplexed pilot and data design shown in Fig.~\ref{pilot_pattern}(a) and defined by
\begin{equation}
\begin{array}{rcl}
s_{k}\left(l\right)&=&
\begin{cases}
...
\noindent With Lemma 1, the correlation matrix of channel estimate
$\hat{\bh}$ is
\begin{equation}
\begin{array}{rcl}
\bR_{\hat{\bH}}&=& \bR_{\bH} + \frac{\sigma_{\mbox{\tiny FL}}^{2}}{L_{c}^2}\bI\\
&=&\sum\limits_{i=1}^{M}\left(\lambda_{i}+\frac{\sigma_{\mbox{\tiny
FL}}^2}{L_{c}^2}\right)\bv_{i}\bv_{i}^{\rm H}
\end{array}
\end{equation}
\noindent and the rate distortion function for eigenvalue
quantization is
\begin{equation}
\begin{array}{rcl}
R_{\lambda}\left(D\right)&\geq&\frac{1}{2}\log_{2}\left(\frac{\sigma_{\mbox{\tiny
FL}}^{2}}{L_{c}^{2}D}+\frac{\sigma_{\lambda}^{2}}{D}\right)
\end{array}.
\end{equation}
...
\subsection{Channel Amplitude Quantization}
...
If we denotes $\bvartheta=\left[\bh^{\rm T}\ \bs_{d}^{\rm
T}\right]^{\rm T}$, the complex FIM is given by
\begin{equation}\hspace{-0.10in}
\begin{array}{rcl}
...
s_{d}}^{\ast}}\right|^2\right\}$. The CRLB on the channel
estimation with either TMP or SIP can therefore be formulated by
the following lemma.
...
\subsection{Channel Phase Quanization}
\begin{figure}
\center{
...
\section{The Reliability of MIMO Feedback Channel}


On the other hand, the minimum achievable rate at a certain
distortion is given by Shannon distortion theory, where the rate
distortion function is
\begin{equation}\hspace{-0.10in}
\begin{array}{rcl}
R(D)&=&\min\limits_{E\{d(\bh,\ \hat{\bh})\leq D\}} I\left(\bH;\
\hat{\bH}\right)
\end{array}.\label{RDF}
\end{equation}
\noindent For a $\bcN\left(0,\ \sigma^2\right)$ source with the
squared error distortion defined in
(\ref{squared_error_distortion}), the minimum achievable rate at
distortion D with no feedforward is
\begin{equation}
\begin{array}{rcl}
...
to each other, the well-known sphere-packing upper bound of
Gaussian channel reliability function is
\begin{equation}%\hspace{-0.00in}
\begin{array}{rcl}
